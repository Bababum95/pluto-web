...
- `pnpm generate:api` — regenerate OpenAPI client/types via Orval
...
- `src/app/ui/` — app shell UI (`devtools`, `full-screen-loader`) used from `main.tsx` / routes
...
- `src/app/store/` — Redux store config, app-level slices, listener middlewares, `registerSyncEntities` (integration tests under `app/store/__tests__/`)
...
- `src/shared/api/generated/**` (generated by `orval`)
